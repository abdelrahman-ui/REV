...
void LCD_Command( unsigned char cmnd )
{
	...
	DIO_Write_Port(Dio_Port_A,((LCD_Data_Port & 0x0F) | (cmnd & 0xF0)));
	Dio_WriteChannel(LCD_RS,STD_low);/* RS=0, command reg. */
	Dio_WriteChannel(LCD_EN,STD_high);		/* Enable pulse */
	_delay_us(10);
	Dio_WriteChannel(LCD_EN,STD_low);
	_delay_us(2000);
	//LCD_Port = (LCD_Port & 0x0F) | (cmnd << 4);/* Sending lower nibble */
	DIO_Write_Port(Dio_Port_A,((LCD_Data_Port & 0x0F) | (cmnd << 4)));
